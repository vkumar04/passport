...
export default {
  ...
  methods: {
    ...
    createFactory(){
      this.formData.children = Array.from({length: this.formData.range}, () => Math.floor(Math.random() * (this.formData.maxRange - this.formData.minRange + 1) + this.formData.minRange));
